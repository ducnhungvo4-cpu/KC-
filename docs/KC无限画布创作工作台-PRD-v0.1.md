...
1. 用户在现有线性系统中选择“上传剧本”进入线性流水线，或选择“画布空间”进入无限画布项目。
2. 用户选择已有项目或新建项目后，系统读取该项目下的画布列表。
3. 如果项目只有 1 张画布，系统自动跳过画布列表并直接进入画布；如果项目有多张子画布，先展示画布列表，由用户选择对应子工程。
4. 用户在线性分镜页选择要精修的分镜视频或素材时，系统带 `project_id`、`shot_id`、可选 `canvas_id`、视频地址、封面和来源记录进入画布。
5. 画布创建或定位视频节点，由视频节点承接分镜信息。
6. 用户在画布中拖拽、连线、上传图片/视频/文本/音频并继续迭代。
7. 图片生成、视频生成、音频生成、裁剪、多角度等操作通过节点发起模型任务。
8. 后端统一代理模型调用，写入积分预扣、确认、失败返还等流水。
9. 用户将满意结果保存到项目素材、新建资产或更新已有资产版本。
10. 项目素材和正式资产回到共享库；二期再按 `shot_id` 回传线性分镜版本。

### 6.1 关键术语

| 术语 | 定义 | 当前原型状态 |
| --- | --- | --- |
| 项目画布 | 绑定一个短剧项目的无限画布空间，承载节点、连线、位置、缩放和保存状态 | 已用项目管理页、画布列表和本地快照模拟 |
| 子画布 | 同一项目下的独立画布，用于拆分团队成员、分镜段落或资产方向，避免所有人共用一张大画布 | 已用画布列表、新建画布和权限弹窗模拟 |
...
| `canvas_id` | 项目下单张画布的唯一标识，用于隔离节点、连线、分组和备份 | 已在节点、连线和画布快照中模拟 |
...
| P0 | 项目管理 | 项目与子画布管理 | 项目与现有项目体系绑定；多画布项目进入画布列表，单画布项目自动跳过列表 |
| P0 | 画布权限 | 子画布权限与只读模式 | 画布支持负责人、可编辑、只读；只读用户只能查看不能修改 |
...
| P0 | 音频节点 | 文本转语音与音频上传 | 新增音频节点，支持上传音频和输入文本生成语音 |
...
| P2 | 协作 | 实时多人协同 | 评论、任务分配、实时协同查看与冲突处理；第一期先用子画布和只读权限降低误操作 |
...
| --- | --- | --- | --- | --- |
| P0-01 | 项目画布 | 项目、子画布与权限管理 | P0 | 用户能以项目为单位进入画布；多画布项目先选子画布，单画布项目自动进入 |
...
| P0-13 | 线性入口 | 画布空间入口 | P0 | 线性系统上传剧本旁边有画布空间入口，分镜页能带字段跳入画布 |
| P0-14 | 音频节点 | 音频上传与语音生成 | P0 | 用户能新建音频节点、上传音频、输入文本生成语音并播放结果 |

### 9.2 P0-01 项目、子画布与权限管理

| 字段 | 内容 |
| --- | --- |
| 功能说明 | 无限画布必须绑定现有项目体系，但项目不再直接等同于一张画布。一个项目可以有一张或多张子画布，用于拆分团队成员、分镜段落或资产方向 |
| 用户入口 | 线性系统项目详情页、分镜页面“画布中精修”按钮、线性系统首页“画布空间”入口、画布工作台项目列表 |
| 核心操作 | 打开项目、查看画布列表、新建子画布、进入子画布、管理画布权限、自动创建默认画布、手动保存、自动保存、查看最后快照状态、按项目组筛选、按项目类型筛选、搜索项目 |
| 系统规则 | 如果项目只有 1 张画布，系统自动跳过画布列表并直接进入画布，保证单人使用体验不变；如果项目有多张子画布，先进入画布列表，由用户选择子工程 |
| 数据范围 | 节点、连线、节点分组、节点位置、节点参数、引用资产、引用素材、生成结果和任务记录均归属于当前 `canvas_id`；资产、素材、分镜和积分仍归属于 `project_id` |
| 权限规则 | 沿用现有项目权限，并在画布层增加负责人、可编辑、只读三类权限；只读用户只能查看，不能新增、拖拽、删除、连线、上传、生成或保存 |
| 保存规则 | 画布状态自动保存，用户也可以手动触发保存；保存失败时需要提示并允许重试 |
| 异常处理 | 无项目权限时禁止进入；无画布权限时禁止进入对应子画布；画布加载失败时展示重试入口；自动保存失败时保留本地临时状态 |
| 原型截图 | S09 项目管理首页，S17 画布列表页，S18 新建画布弹窗，S19 权限管理弹窗，S01 画布总览 |
| 验收标准 | 多画布项目不会直接进入画布；单画布项目必须自动跳过列表；同一项目下不同画布的数据互不覆盖；刷新页面后当前画布节点位置和连线不丢失 |
...
| 支持节点 | 文本节点、图片节点、视频节点、音频节点；资产和素材进入画布后按文件/资源类型转换成对应节点，其中视频节点可携带线性系统分镜信息 |
| 基础操作 | 缩放、平移、拖拽节点、框选、多选、复制、粘贴、删除、撤销预留、右键菜单、小地图定位、整理画布 |
| 连线规则 | 节点之间可通过连接端口建立引用关系；连线表示“上游节点作为下游节点的输入或参考” |
| 输入规则 | 图片、视频、文本、音频、资产均可作为其他生成节点的入参；不同节点根据能力限制接受的输入类型 |
...
### 9.14 P0-13 线性系统画布空间入口

| 字段 | 内容 |
| --- | --- |
| 功能说明 | 画布产品需要嵌入现有线性工作流，不让业务在两个系统之间重新找入口。在线性系统项目首页，上传剧本旁边增加“画布空间”入口 |
| 用户入口 | 线性系统首页“上传剧本”旁边的“画布空间”按钮；线性分镜页“到画布微调”按钮 |
| 入口区分 | 用户点击“上传剧本”继续进入原线性流水线；点击“画布空间”进入画布项目创建或选择流程 |
| 跳转字段 | `project_id`、`shot_id`、可选 `canvas_id`、`entry_source`、`video_url`、`cover_url`、`shot_name`、`return_url` |
| 默认规则 | 如果传入 `canvas_id`，打开指定画布；如果没有传入 `canvas_id`，单画布项目直接进入默认画布，多画布项目进入画布列表 |
| 一期边界 | 第一期保证线性数据能传入画布；画布结果按 `shot_id` 回传线性系统放到第二期 |
| 原型截图 | S20 线性系统画布空间入口参考，S02 分镜视频节点详情 |
| 验收标准 | 线性首页能清楚区分画布项目和上传剧本流程；分镜页入口能带字段跳入画布；画布能识别 `project_id`、`shot_id` 和可选 `canvas_id` |

### 9.15 P0-14 音频节点与语音生成

| 字段 | 内容 |
| --- | --- |
| 功能说明 | 节点列表新增音频节点，用于本地音频承接和文本转语音。音频节点和图片、视频、文本一样，是标准节点 |
| 用户入口 | 添加节点面板、画布右键菜单、统一上传入口、连线快速菜单“上传” |
| 节点空态 | 节点主体显示音频占位和上传入口；下方参数面板显示“输入要合成的文本”的文本框 |
| 支持操作 | 新建音频节点、上传本地音频、输入文本、选择语音模型、插入停顿/语气词标签、调整基础参数、生成语音、播放、下载、保存到项目素材 |
| 面板信息 | 模型选择默认可用 `Minimax-speech-2.8-hd`；显示字数统计、积分预估、生成按钮和生成加载态 |
| 结果规则 | 生成成功后，音频结果写入当前音频节点；如果从上传入口上传音频，则创建标准音频节点 |
| 原型截图 | S21 音频节点入口菜单，S22 音频生成参数面板 |
| 验收标准 | 音频节点可从节点列表、右键菜单、本地上传创建；输入文本后可发起语音生成；生成中有加载态；成功后能播放、下载、保存；不影响图片、视频、文本节点原有操作 |
...
| 项目下多画布 | 正式版需要 `canvases`、`canvas_permissions`，并让节点、连线、分组全部带 `canvas_id` | 原型已用本地存储模拟项目画布列表、新建画布、权限和只读模式 | PRD 标为 P0 目标，研发接后端时按字段表改造 |
...
| 音频节点 | 音频节点支持上传音频和文本转语音，作为后续视频合成素材 | 原型已提供音频节点、音频上传承接和文本转语音入口，正式模型接口仍需接入 | PRD 标为 P0 目标，研发需补语音模型代理和积分记录 |
...
| `role` 枚举 | 说明 |
| --- | --- |
| `owner` | 画布负责人，可编辑画布、管理权限、保存和导出备份 |
| `editor` | 可编辑画布，可新增、拖拽、连线、生成、上传和保存 |
| `viewer` | 只读查看，不能产生任何写操作 |
...
| `audio` | 音频节点，承载本地上传音频和文本转语音结果 |
| `asset` | 后端资产对象类型；一期进入画布后不单独展示为资产节点，而是转换为标准图片节点并保留 `source=asset_library` |
| `material` | 后端素材对象类型；进入画布后按素材类型转换为图片、视频、文本或音频节点 |

### 10.4 视频节点携带分镜信息 data 字段
...
| S17 | 项目下画布列表 | 展示一个项目下存在多个子画布时，先进入画布列表而不是直接进入画布 |
| S18 | 新建子画布弹窗 | 展示用户在项目下新建一个子画布的入口、命名和进入规则 |
| S19 | 画布权限管理弹窗 | 展示项目下不同子画布的拥有者、编辑者、只读权限设置 |
| S20 | 线性系统画布空间入口参考 | 展示现有线性系统项目首页中，“上传剧本”旁边应新增“画布空间”入口的位置 |
| S21 | 音频节点入口菜单 | 展示节点菜单新增音频节点，用户可以从画布右键菜单创建音频节点 |
| S22 | 音频节点参数面板 | 展示音频节点的文本输入、模型选择、停顿/语气词、积分和生成入口 |
...
| `21-canvas-list-page.png` | 项目下画布列表页，展示多子画布入口、最近快照和权限入口 | `2048x1118` | P0-01、S17 |
| `22-new-canvas-modal.png` | 新建子画布弹窗，展示画布命名和创建后进入规则 | `2048x1118` | P0-01、S18 |
| `23-canvas-permission-modal.png` | 画布权限管理弹窗，展示拥有者、可编辑、只读权限字段 | `2048x1118` | P0-01、S19 |
| `24-audio-context-menu.png` | 音频节点入口菜单，展示节点列表中新增音频节点 | `2048x1118` | P0-02、P0-14、S21 |
| `25-audio-node-panel.png` | 音频节点参数面板，展示文本转语音输入框、模型和生成控件 | `2048x1118` | P0-14、S22 |
| `26-linear-workflow-entry-reference.jpg` | 线性系统项目首页参考图，用于标注上传剧本旁边新增画布空间入口 | `2560x1245` | P0-13、S20 |
...
![项目下画布列表](prd-screenshots/21-canvas-list-page.png)

#### 新建子画布弹窗

![新建子画布弹窗](prd-screenshots/22-new-canvas-modal.png)

#### 画布权限管理弹窗

![画布权限管理弹窗](prd-screenshots/23-canvas-permission-modal.png)

#### 线性系统入口参考

![线性系统入口参考](prd-screenshots/26-linear-workflow-entry-reference.jpg)

#### 音频节点入口菜单

![音频节点入口菜单](prd-screenshots/24-audio-context-menu.png)

#### 音频节点参数面板

![音频节点参数面板](prd-screenshots/25-audio-node-panel.png)
...
| R10 | 项目不再强绑定单一画布 | 团队项目可以在项目下拆多个子画布；如果项目只有一个子画布，系统自动跳过列表页直接进入画布 |
| R11 | 画布权限先做画布级控制 | 一期支持拥有者、可编辑、只读三种权限；只读用户可查看和平移缩放，但不能编辑、生成、保存 |
| R12 | 音频节点是标准节点 | 音频节点和图片、视频、文本一样进入节点列表、右键菜单、统一上传和素材复用链路，不做孤立功能面板 |
