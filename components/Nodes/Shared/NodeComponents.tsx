...
export const ThumbnailItem = memo(({ src, index, isDark }: { src: string, index: number, isDark: boolean }) => {
    ...
    return (
        <div className={`relative w-[48px] h-[48px] flex-shrink-0 border rounded-lg overflow-hidden shadow-sm group/thumb cursor-pointer hover:border-[#4446CE]/50 transition-colors ${isDark ? 'border-zinc-700 bg-black/40' : 'border-gray-300 bg-gray-100'}`}>
...
export const CustomDropdown = ({ 
    ...
    const activeItem = isDark ? 'bg-[#4446CE]/10 text-[#8F91F4]' : 'bg-[#F0F1FF] text-[#4446CE]';
    ...
    return (
        <div className="relative h-full flex items-center" ref={ref}>
            <div className={`flex items-center gap-1.5 cursor-pointer group h-full px-1.5 rounded transition-colors ${isOpen ? (isDark ? 'bg-white/5' : 'bg-gray-100') : ''} ${hoverClass}`} onClick={(e) => { e.stopPropagation(); onToggle(); }}>
                {Icon && <Icon size={13} className={`transition-colors ${isOpen ? 'text-[#8F91F4]' : iconColor}`} />}
                ...
                {!Icon && <Icons.ChevronRight size={10} className={`transition-all duration-200 ${isOpen ? 'rotate-[-90deg] text-[#8F91F4]' : `rotate-90 ${isDark ? 'text-zinc-600 group-hover:text-zinc-400' : 'text-gray-400 group-hover:text-gray-600'}`}`} />}
            </div>
            {isOpen && (
                    ...
                    {options.map((opt: any) => {
                        ...
                        return (
                            <div key={opt} className={`px-3 py-1.5 text-[11px] font-medium transition-colors flex items-center justify-between group/item ${isDisabled ? 'text-zinc-600 cursor-not-allowed opacity-50' : opt == value ? activeItem + ' cursor-pointer' : (isDark ? 'text-zinc-400' : 'text-gray-500') + ` ${optionHover} cursor-pointer`}`} onClick={(e) => { e.stopPropagation(); if (!isDisabled) { onChange(opt); onClose(); } }}>
                                ...
                                {opt == value && <Icons.Check size={10} className="text-[#8F91F4] shrink-0 ml-2" />}
                            </div>
                        );
                    })}
                </div>
